...
众所周知，计算机中的有符号整数（比如`int`, `long`等）在底层是用补码表示的，而补码的规则是：

- 正数的补码就等于其原码
- 负数的补码等于其「原码的数值位逐位取反」然后「加一」

那么我们把正的 $n$ 改成 $-n$，不就等同于做「将 $n$ 取反后加一」这个操作吗？例如，补码形式的 44 为$(0,101100)$，-44 的补码形式为$(1,010011)$，则 

$$
(44)_{10} \& (-44)_{10} = (0,101100)_补\&(1,010011)_补=(0,000100)_补=(4)_{10}
$$
...
在前面的部分，我们已经讨论了如何通过树状数组实现**单点修改**和**区间查询**。接下来，我们将讨论如何通过树状数组实现**区间修改**和**单点查询**。

区间修改的核心思想是利用**差分数组**。假设我们有一个数组 $a$，我们可以构造一个差分数组 $d$，其中 $d[i] = a[i] - a[i-1]$（$d[1] = a[1]$）。通过差分数组，我们可以将区间修改操作转化为单点修改操作。

例如，如果我们想要将区间 $[l, r]$ 中的每个元素都加上 $k$，我们只需要在差分数组 $d$ 中进行如下操作：

1. 将 $d[l]$ 加上 $k$
2. 将 $d[r+1]$ 减去 $k$

这样，当我们查询某个位置 $i$ 的值时，只需要求差分数组 $d$ 的前缀和即可。
...
单点查询的实现非常简单，只需要求差分数组 $d$ 的前缀和即可。由于我们已经通过树状数组维护了差分数组 $d$，因此单点查询的时间复杂度为 $O(\log n)$。
...
在前面的部分，我们已经讨论了如何通过树状数组实现**区间修改**和**单点查询**。接下来，我们将讨论如何通过树状数组实现**区间修改**和**区间查询**。

为了实现区间修改和区间查询，我们需要维护两个树状数组：一个用于维护差分数组 $d$，另一个用于维护 $i \cdot d[i]$ 的前缀和。通过这两个树状数组，我们可以高效地实现区间修改和区间查询。

区间修改的实现与之前类似，我们只需要在差分数组 $d$ 中进行单点修改即可。
...
区间查询的实现稍微复杂一些。我们需要利用两个树状数组来计算区间和。具体来说，区间 $[l, r]$ 的和可以通过以下公式计算：

$$
\sum_{i=l}^{r} a[i] = \sum_{i=1}^{r} a[i] - \sum_{i=1}^{l-1} a[i]
$$

而 $\sum_{i=1}^{x} a[i]$ 可以通过以下公式计算：

$$
\sum_{i=1}^{x} a[i] = (x + 1) \cdot \sum_{i=1}^{x} d[i] - \sum_{i=1}^{x} i \cdot d[i]
$$

因此，我们需要维护两个树状数组，一个用于维护 $\sum_{i=1}^{x} d[i]$，另一个用于维护 $\sum_{i=1}^{x} i \cdot d[i]$。
...
- [LuoguP3372【模板】线段树 1(区间修改 & 区间查询)](https://www.luogu.com.cn/problem/P3372)
